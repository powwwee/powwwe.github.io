...
error_reporting(0);
    ...
    if (empty($_SESSION['steam_uptodate']) or $_SESSION['steam_uptodate'] == false or empty($_SESSION['steam_personaname'])) {
        ...
        $pFCurl = curl_init();
        $pProfileData = "http://api.steampowered.com/ISteamUser/GetPlayerSummaries/v0002/?key=".$steamauth['apikey']."&steamids=".$_SESSION['steamid'];
        curl_setopt($pFCurl, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($pFCurl, CURLOPT_URL, $pProfileData);
        $pProf = curl_exec($pFCurl);
        $content = json_decode($pProf, true);
        curl_close($pFCurl);
